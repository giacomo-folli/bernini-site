<div class="overflow-hidden rounded-3xl bg-black px-8 py-24 shadow-2xl">
	<div class="mx-auto max-w-2xl text-center">
		<span class="mb-6 inline-flex items-center rounded-full bg-white/10 px-3 py-1 text-sm text-white"
			>Inizia oggi</span
		>
		<h2 class="text-3xl font-light tracking-tight text-white sm:text-4xl lg:text-5xl">
			Pronto a iniziare il tuo <span class="font-medium">viaggio?</span>
		</h2>
		<p class="mx-auto mt-6 max-w-xl text-lg leading-relaxed text-zinc-300">
			Unisciti a una community di persone appassionate che stanno trasformando il loro modo di muoversi e
			vivere.
		</p>
		<div class="mt-10 flex items-center justify-center gap-6">
			<a
				href="/come-funziona"
				class="rounded-full bg-white px-8 py-4 text-sm font-semibold text-black shadow-lg transition-all duration-300 hover:bg-zinc-100 hover:translate-y-[-2px]"
			>
				Scopri i corsi
			</a>
			<a
				href="/contatti"
				class="rounded-full border border-white/25 px-8 py-4 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:bg-white/10 hover:translate-y-[-2px]"
			>
				Contattami
			</a>
		</div>
	</div>
</div>