							...
							<div class="mx-auto max-w-xl pb-6">
								<div>
									{#if testimonialActive === 1}
										...
										>
											Grazie a Francesco ho riscoperto la gioia di muovermi. Il suo approccio unico
											ha trasformato non solo il mio corpo, ma anche la mia mentalità.
										</p>
									{/if}
								</div>
								<div>
									{#if testimonialActive === 2}
										<p
											class="mx-auto flex items-center gap-2 text-balance text-center text-base font-medium leading-6 text-zinc-500"
										>
											Le sessioni con Francesco sono sempre stimolanti e divertenti. Ho guadagnato
											forza, flessibilità e una nuova confidenza nei miei movimenti.
										</p>
									{/if}
								</div>
							</div>
							...
							<div class="flex justify-center px-4 pb-6 pt-4 md:py-6">
								<div class="text-center">
									{#if testimonialActive === 1}
										...
										<a href="#" class="text-xs text-zinc-500">@marco.z</a>
									{/if}
								</div>
								<div class="text-center">
									{#if testimonialActive === 2}
										<h2 class="text-sm font-medium text-zinc-500 lg:text-base">Giulia Santi</h2>
										<a href="#" class="text-xs text-zinc-500"> @assereje</a>
									{/if}
								</div>
							</div>
						</div>
